                                        ...
                                        PicturePile& picture)
{
    ...
    /// M: Fix canvas background for GL Canvas @{
    bool hasDecorations = false;
#if ENABLE(MTK_GLCANVAS)
    if (layer->subclassType() == LayerAndroid::CanvasLayer) {
        RenderLayer* renderLayer = renderLayerFromClient(m_client);
        RenderObject* renderObj = renderLayer ? renderLayer->renderer() : 0;
        if (renderObj) {
            hasDecorations = (renderObj->hasBoxDecorations()
                                || (renderObj->style() && renderObj->style()->outlineWidth())
                                || (renderObj->style() && renderObj->style()->hasBorderRadius()));
        }
    }
#endif
    PicturePileLayerContent* content = new PicturePileLayerContent(picture, hasDecorations);
    /// @}
