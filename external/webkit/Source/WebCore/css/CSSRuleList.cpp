...
#if USE_RULELIST_POOL
PassRefPtr<CSSRuleList> CSSRuleListPool::getCSSRule(StyleList* list)
{
    PassRefPtr<CSSRuleList> passRuleList;
    CSSRuleList* ruleList = m_ruleMap.get(list);
    if (!ruleList) {
        passRuleList = CSSRuleList::create(list, false);
        m_ruleMap.set(list, passRuleList.get());
        return passRuleList;
    }
    ruleList->ref();
    return adoptRef(ruleList);
}

void CSSRuleListPool::removeCSSRule(StyleList *list)
{
    if (m_ruleMap.contains(list)) {
        m_ruleMap.remove(list);
    }
}

CSSRuleListPool* CSSRuleListPool::getInstance()
{
    if (!gCSSRuleListPool) {
        gCSSRuleListPool = new CSSRuleListPool();
    }
    return gCSSRuleListPool;
}

CSSRuleListPool* CSSRuleListPool::gCSSRuleListPool = 0;
#endif
} // namespace WebCore
