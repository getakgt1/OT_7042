...
#define USE_RULELIST_POOL 1

#if USE_RULELIST_POOL
class CSSRuleListPool {
public:
    static CSSRuleListPool* getInstance();
    PassRefPtr<CSSRuleList> getCSSRule(StyleList* list);
    void removeCSSRule(StyleList* list);
private:
    CSSRuleListPool() {};

    HashMap<StyleList*, CSSRuleList*> m_ruleMap;
    static CSSRuleListPool* gCSSRuleListPool;
};
#endif
