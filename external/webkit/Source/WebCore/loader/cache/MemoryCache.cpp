...
void MemoryCache::dumpStats()
{
    ...
    XLOG("%-13s %-13s %-13s %-13s %-13s %-13s %-13s\n", "", "Count", "Size", "LiveSize", "DecodedSize", "PurgeableSize", "PurgedSize");
    XLOG("%-13s %-13s %-13s %-13s %-13s %-13s %-13s\n", "-------------", "-------------", "-------------", "-------------", "-------------", "-------------", "-------------");
    XLOG("%-13s %13d %13d %13d %13d %13d %13d\n", "Images", MEMORY_CACHE_ARGS(s.images));
...
void MemoryCache::dumpStats_JsonFormat(char* buf)
{
    if (!buf)
        return;
    ...
    sprintf(buf, "    \"MemoryCache\": {\r\n"
                 ...
                 "        }\r\n"
                 "    }",
        liveCapacity(), deadCapacity(),
        MEMORY_CACHE_ARGS(s.images),
        MEMORY_CACHE_ARGS(s.cssStyleSheets),
#if ENABLE(XSLT)
        MEMORY_CACHE_ARGS(s.xslStyleSheets),
#endif
        MEMORY_CACHE_ARGS(s.scripts),
        MEMORY_CACHE_ARGS(s.fonts));
}
