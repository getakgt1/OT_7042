...
void RenderLayer::updateRepaintRectsAfterScroll(bool fixed)
{
    if (fixed || renderer()->style()->position() == FixedPosition) {
        computeRepaintRects();
        fixed = true;
    } else if (renderer()->hasTransform() && !isRootLayer()) {
                        ...
                        PaintLayerFlags paintFlags)
{
    ...
    if (renderer()->document()->didLayoutWithPendingStylesheets() && !isRootLayer() && !renderer()->isRoot())
        return;
...
bool RenderLayer::hitTest(const HitTestRequest& request, HitTestResult& result)
{
    ...
    if (!insideLayer) {
        ...
        if ((request.active() || request.mouseUp()) && isRootLayer()) {
